      ...
      series.forEach((lk) => {
        ...
        if (!tempMap[name]) {
          ...
          tempMap[name] = {
            ...
            type: lk.type,
            yAxis: lk.yAxis,
            tooltip: getTooltipDataLabels(lk.tooltipLabelFormat),
            dataLabels: getCombinationChartDataLabels(lk.tooltipLabelFormat)
          };
...
getDataLabels = (options) => {
  let result = {
    ...
    formatter: function () {
      ...
      if (options.yaxisLabelFormat == '$') {
        result = options.yaxisLabelFormat + result;
      } else if (options.yaxisLabelFormat == '%') {
        result = this.y + options.yaxisLabelFormat;
      }
      return result;
    },
    color: 'black',
  ...
  let response = result;
  console.log('getDataLabels formatter=', response);
  return response;
};
  ...
  let result = {
    ...
    formatter: function () {
      ...
      if (options.yaxisLabelFormat == '$') {
        ...
        result = this.value + options.yaxisLabelFormat;
      }
      return result;
    }
  };
...
getCombinationChartDataLabels = (format) => {
  let result = {
    enabled: true,
    formatter: function () {
      this.y = Math.round(this.y);
...
getTooltipDataLabels = (format) => {
  let result = {
    ...
    pointFormatter: function () {
      ...
      if (format == '$') {
        result = this.series.name + ': ' + format + result + ' ';
      } else if (format == '%') {
        result = this.series.name + ': ' + this.y + format + ' ';
      }
      return result;
    }
  };
