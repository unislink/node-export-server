 ...
 * algorithm
 * @export utils
 * @param fn {any} A function to try to call with the backoff algorithm on
 * @param attempt {number} The number of an attempt, where the first one is 0
 */
const expBackoff = async (fn, attempt = 0, ...args) => {
  try {
...
/** Check if string contains private range urls
 * @export utils
 * @param item {string} item to be checked
 */
const isPrivateRangeUrlFound = (item) => {
  return [
    'localhost',
    '(10).(.*).(.*).(.*)',
    '(127).(.*).(.*).(.*)',
 ...
 * @param absoluteProps {any} Array of object names that should be force merged
 */
const mergeConfigOptions = (options, newOptions, absoluteProps = []) => {
  ...
  if (customCode && typeof customCode === 'string') {
    ...
    if (customCode.endsWith('.js')) {
      return allowFileResources
        ? wrapAround(readFileSync(path.join(__basedir, customCode), 'utf8'))
        : false;
    } else if (
      !(
