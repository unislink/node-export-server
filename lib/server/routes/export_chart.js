...
const { computeChartOptions } = require('./../../chart_utils.js');
...
const exportHandler = (request, response) => {
  ...
  let chartOptions = computeChartOptions(inputData);
  let instr = isCorrectJSON(chartOptions);
...
module.exports = (app) => {
  app.post('/export/chart', exportHandler);
};
